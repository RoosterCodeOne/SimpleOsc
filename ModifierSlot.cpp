    ...
    : slotIndex(index), processor(proc)
{
    ...
    if (slotIndex == 1) {
        ...
        breathToggle->setToggleState(true, juce::dontSendNotification);
        ...
        breathDepthSlider->setRange(0.0, 1.0, 0.01);
        breathDepthSlider->setValue(0.5);
...
void ModifierSlot::resized() {
    ...
    if (slotIndex == 0 && offsetSlider && widthSlider && binauralToggle) {
    ...
    else if (slotIndex == 1 && breathRateSlider && breathDepthSlider && breathToggle) {
        ...
        juce::Rectangle<int> col1 = area.removeFromLeft(colWidth);
        area.removeFromLeft(spacing);
        juce::Rectangle<int> col2 = area;

        breathRateSlider->setBounds(col1);
        breathDepthSlider->setBounds(col2);
    }

   else if (slotIndex == 2 && harmonicLevelSliders.size() == 8) {
        int padding = 4;
        int boxSize = (area.getWidth() - padding * 3) / 4;
        int rowHeight = (area.getHeight() - padding) / 2;

        for (int i = 0; i < 8; ++i) {
            int row = i / 4;
            int col = i % 4;
            int x = col * (boxSize + padding);
            int y = row * (rowHeight + padding);
            harmonicLevelSliders[i]->setBounds(x, y, boxSize, rowHeight);
        }
    }
